...
// use this to hold our data rather than passing stuff around
class sdf_data {
	...
	private static $DONOR_TEMPLATE_ID = '00X50000001VaHS';
	...
	// set up data['amount-cents'] and ['amount-string']
	...
	private function set_amount() {
		if(strpos($this->data['donation'], 'custom') !== false) {
			$this->data['custom'] = $this->IS_CUSTOM;
			...
			$this->data['custom'] = $this->IS_NOT_CUSTOM;
			$donated_value = get_std_amount();
		}

		$this->data['amount-cents'] = (int) $donated_value * 100;
	...
	private function set_recurrence() {
		...
		$this->data['recurrence-string'] = $string;
	}

	// Param: sum is the sum of other donation li's
	// that are passed here from the set member level
	// function in the SalesForce section
	// XXX don't call this function twice - move it to SalesForce section
	private function set_member_level($sum = 0) {
		if($sum > 0) {
			$amount = $sum;
		} else {
		...
		$type = $this->data['recurrence-type'];
		$level = 'Donor';
		...
		$this->data['membership-level'] = $level;
	}
	...
	// It's never called from anywhere!
	// Should only be executed once, when the plugin is first installed
	// and the Stripe account is new, or when switching from test to live
	// for the first time.
	...
	private function charge() {
		if($this->data['recurrence-type'] == $this->ONE_TIME) {
			single_charge();
		} else {
			recurring_charge();
		}
	...
	private function recurring_charge() {
		if($this->data['custom'] == $this->IS_CUSTOM) {
			custom_plan();
		} else {
			std_plan();
		}
	...
	private function custom_plan() {
		$plan_id = strtolower($this->data['recurrence-string']) 
			. '-' . ($this->data['amount-cents'] / 100);

		try {
			$plan = Stripe_Plan::retrieve($plan_id);
		} catch(Stripe_Error $e) {
			$recurrence = ($this->data['recurrence-type'] == $this->ANNUAL ? 'year' : 'month');
	...
	// XXX may need to update the validity tester because we're not returning the object anymore
	public function salesforce_api($input = null) {
	...
	private function get_contact() {
		...
		if($id !== null) {
			...
			try {
				$contact = array_pop($this->sf_cnxn->retrieve($fieldlist, 'Contact', array($id)));
			} catch(Exception $e) {
	...
	private function search_salesforce() {
		...
		try {
			$response = $this->sf_cnxn->search($query);
		} catch(Exception $e) {
			sdf_message_handler('log', __FUNCTION__ . ' : ' . $e->faultstring);
		}
	...
	private function get_donations() {
		...
		if($this->sf_contact->Id !== null) {
			...
			try {
				$response = $this->sf_cnxn->query($query);
	...
	private function update() {
		// TODO check values before clobbering them
		recalc_member();
		set_constant();
		company();
		name();
	...
	// Also sets contact->paid__c
	private function recalc_member() {
		foreach($this->sf_donations as $donation) {
			$sum += $donation['amount-cents'];
		}

		if($this->data['recurrence-type'] == $this->MONTHLY) {
			$times = 11 - intval(date('n'));
			$sum += $times * $this->data['amount-cents'];
		} else {
			$sum += $this->data['amount-cents'];
		}

		if($sum >= 7500) {
			$this->data['is-member'] = $IS_MEMBER;
		} else {
			$this->data['is-member'] = $IS_NOT_MEMBER;
		}

		member_level($sum);
		$this->sf_contact->Paid__c = $sum;
	}
		...
		if(!isset($this->sf_contact->AccountId)) {
			if(!empty($this->data['company'])) {
				...
				try {
					...
					$search = $sforce->upsert('Name', array($company), 'Company');
					$this->sf_contact->AccountId = $search[0]->id;
				} catch(Exception $e) {
					sdf_message_handler('log', __FUNCTION__ . ' : ' . $e->faultstring);
					$this->sf_contact->AccountId = $this->FRIEND_OF_SPARK;
				}
	...
	private function type() {
		$type = $this->data['is-member'] == $this->IS_MEMBER ? 'Spark Member' : 'Donor';
		$this->sf_contact->Type__c = $type;
	}

	private function start_date() {
		if(!isset($this->sf_contact->Membership_Start_Date__c)) {
			$this->sf_contact->Membership_Start_Date__c = date($this->SF_DATE_FORMAT);
		}
	}

	// XXX is it always one year from today? Otherwise we could
	// be going way into the future, and I don't think that's right
	// What about one time donors?
	private function renewal_date() {
		if(isset($this->sf_contact->Renewal_Date__c)) {
			$date = date($this->SF_DATE_FORMAT, strtotime(
				$this->sf_contact->Renewal_Date__c . ' +1 year'));
		} else {
			$date = date($this->SF_DATE_FORMAT, strtotime('+1 year'));
		}
		$this->sf_contact->Renewal_Date__c = $date;
	}

	private function member_level() {
		$this->sf_contact->Member_Level__c = $this->data['membership-level'];
	}

	private function first_active() {
		if(!isset($this->sf_contact->First_Active_Date__c)) {
			$this->sf_contact->First_Active_Date__c = date($this->SF_DATE_FORMAT);
		}
	}
	...
	private function upsert() {
		if(isset($this->sf_contact->Id)) {
			// update on id.
			try {
				$reponse = $sforce->update(array($this->sf_contact), 'Contact');
			} catch(Exception $e) {
				sdf_message_handler('log', __FUNCTION__ . ' : ' . $e->faultstring);
			}
		} else {
	...
	private function new_donation() {
		...
		$donation->Amount__c = $this->data['amount-cents'] / 100;
		$donation->Description__c = strlen($this->sf_txn) > 255 ? substr($this->sf_txn, 0, 252) . '...' : $this->sf_txn;
		$donation->Donation_Date__c = date($this->SF_DATE_FORMAT);
		$donation->Type__c = 'Membership';

		try {
			$response = $sforce->create(array($donation), 'Donation__c');
		} catch(Exception $e) {
			sdf_message_handler('log', __FUNCTION__ . ' : ' . $e->faultstring);
		}
	}
	...
	private function send_email() {
		...
		$donor_email->setTemplateId($this->DONOR_TEMPLATE_ID);
		$donor_email->setTargetObjectId($this->sf_contact->Id);
		$donor_email->setReplyTo($this->REPLY_TO);
		$donor_email->setSenderDisplayName($this->DISPLAY_NAME);
...
EOF;
	...
	private function hdyh() {
		...
		if(isset($this->sf_contact->How_did_you_hear_about_Spark__c)) {
			if(isset($this->data['hearabout-extra'])) {
